...
void sort_vizinhos(Casa* vizinhos, int size)
{
    /* Pela distancia */
    for (int i = 0; i < size - 1; i++)
    {
        ...
        {
            if (vizinhos[j].d > vizinhos[j + 1].d)
            {
                SWAP(vizinhos[j], vizinhos[j + 1]);
            }
        }
    }
}
...
int main()
{
    ...
    */
    ...
    /* Imprime o labirinto, mapa e caminho somente para teste */
    print_maze(maze);
    print_map(visited);
    print_path(path);
